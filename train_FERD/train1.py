...
def main():
    ...
    parser.add_argument('--train_data_path', type=str, default=r"D:\dev\experimental_results\experimentation\input\input260x348_bg_crops64x64_stride48_porcine")
    parser.add_argument('--test_data_path', type=str, default=r"D:\dev\experimental_results\experimentation\input\input130x174_porcine")
    ...
    parser.add_argument('--mse_loss', type=float, default=0.1, help='MSE loss coefficient')
    parser.add_argument('--cos_loss', type=float, default=1.0, help='COS loss coefficient')
    ...
    # build save path
    args.save_dir = os.path.join('./result/', 'seed' + str(args.seed))
    epoch_write_dir = os.path.join(args.save_dir, 'epoch')
    if not os.path.exists(epoch_write_dir):
        os.makedirs(epoch_write_dir)
    
    args.saved_models_dir = os.path.join('./result/', 'seed' + str(args.seed) + '/saved_models')
...
def train(args, convh, classifier, encoder, bn, decoder, deconvh, epoch, train_loader, optimizer, log):
    ...
    for (data) in tqdm(train_loader):
        ...
        loss = 0.1*BceLoss(model_output, labels) + args.mse_loss * MseLoss(data, output_dec) + args.cos_loss * CosLoss(f_abc_enc, f_abc_dec)
