...
from utils.utils import time_string, convert_secs2time, AverageMeter, print_log, save_checkpoint, write_eval_result, \
    write_mean_result, set_seed, seed_fix, get_admap, seed_torch
...
def main():
    ...
    parser.add_argument('--train_data_path', type=str, default=r"C:\Users\alexa\PycharmProjects\Hyperspectral-Anomaly-Detection\porcine_background_crops")
    parser.add_argument('--test_data_path', type=str, default=r"C:\Users\alexa\PycharmProjects\Hyperspectral-Anomaly-Detection\porcine_background_crops_sources")
    parser.add_argument('--input_channel', type=int, default=31, help='the spectral channel number of input HSI')
    ...
    parser.add_argument('--checkpoint_dir', type=str, default=r'train_FERD/result/seed_10/saved_models')
    parser.add_argument('--mse_loss', type=float, default=1.0, help='MSE loss coefficient')
    parser.add_argument('--ssim_loss', type=float, default=0.01, help='SSIM loss coefficient')
    ...
    args.saved_models_dir = args.checkpoint_dir
    ...
    train_dataset = HADDataset(dataset_path=args.train_data_path)
    ...
    test_dataset = HADTestDataset(dataset_path=args.test_data_path)
...
def train(args, convh, classifier, encoder, epoch, train_loader, optimizer, log, spa_fen, frn):
   ...
   # model.train()
    ...
    for (data) in tqdm(train_loader):
        ...
        loss_re = args.mse_loss * MseLoss(data_r, data) + args.ssim_loss * ssim_loss(data_r, data)
